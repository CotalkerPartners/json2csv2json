...
function checkHeaderName(rowName: string): void {
  ...
  for (let i = 0, modesSize: number = rowName.length; i < modesSize; i += 1) {
    switch (rowName[i]) {
      case `[`:
...
function indexErrors(nestMap: INestingMap, rowName: string): void {
  ...
  for (let i = 0, modesSize: number = nestMap.tokens.length; i < modesSize; i += 1) {
...
export function rowClassify(rowName: string, schema: object): Node[] {
    // check format errors
  checkHeaderName(rowName);

  const rowHierarchy: Node[] = [];
  const parentList: (string | number)[] = [];
  const nestMap: INestingMap = tokenizeClassifier(rowName);

    // check for number errors in nestMap for Array type
  indexErrors(nestMap, rowName);
  const modesSize: number = nestMap.modes.length;
  for (let i = 0; i < modesSize; i += 1) {
    const nNode: Node = new Node(i - 1);
  ...
  const lastNode: Node = new Node(modesSize - 1);
  if (modesSize > 1) {
